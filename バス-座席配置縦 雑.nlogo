...
to get-on ;乗車・座席立席位置探し
  set left-rigth random 1 ; 乗客が左右の向きをランダムに決める
  ask humans with [pycor < 4] [setxy 2.25 3] ;バス停の客をバスの入口まで移動させる

  if (pycor)
  ;座席・立席探し
  ;ifelse (towardsxy -8 3 > 0) ;左右の向きを決定
   ; [ask humans with [pycor >= 3] [facexy -1 3]]
   ; [ask humans with [pycor >= 3] [facexy 1 3]]

  ;if (count signs-on humans < 19) ;適当に移動
  ;  [ask humans with [pycor >= 3] [fd 1]]

  if (count humans with [signs-on humans != 0] > 0) ;近くの座席が空いていれば座る
    [ask humans with [pycor >= 3] [move-to one-of signs with [signs-on humans != 0]]]

  if (count signs-on humans > 18) ;人がいない立席に移動
    [ask humans with [pycor >= 3] [move-to one-of neighbors4 with [count humans != 0]]]
...
to get-off ;降車
  ...
  ask humans with [get-off-count = 0] [setxy -11 1]
  ask humans [if (pycor < 2 and pxcor < -10) [die]]
end
